...
r.dtSummary  <- reactive({
  ...
  dtS <- dt0 [ , .(
    ...
    # periodName=paste(input$date1, input$date2),
    MIN=min(value, na.rm = T),
    MEAN=mean(value, na.rm = T) %>% round(1), 
        MAX=max(value, na.rm = T),
    # SD=sd(value, na.rm = T) %>% round(1), 
    # skew=skewness(value, na.rm = T)%>% round(1),  
    # kurtosis=kurtosis(value, na.rm = T)%>% round(1),
    
    # intercept=lm(value ~ as.integer( Date - min(Date)),  na.action=na.omit)$coefficients[1] %>% round(1),
    Dynamics=( lm(value ~ as.integer( Date - min(Date)),  na.action=na.omit)$coefficients[2] * 7 ) %>% round(1)
  ),  
...
radioButtons("show_table", "Show",
             choices = c("Summary", "Raw Data" ),
             selected = "Summary" , inline=T)


DT::renderDataTable(
  if (input$show_table == "Summary" )  {
    r.dtSummary() %>% datatable.title("Summary (Deaths in a million per week)")
  } else {
    r.dt0() %>% datatable.title("Weekly statistics (Deaths in a million per week)")
  }
